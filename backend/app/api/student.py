...
def _compact_progress_report(student: User) -> dict:
    ...
    forum_posts = ForumPost.query.filter_by(author_id=student.id).count()
...
def dashboard(current_user: User):
    ...
    return {
        ...
        'summary': {
            ...
            'forum_posts': len(current_user.posts),
        },
...
def list_modules(current_user: User):
    requested_group = request.args.get('age_group') or current_user.age_group or 'middle'
...
def get_lesson(current_user: User, lesson_id: int):
    ...
    idx = next((i for i, item in enumerate(module.lessons) if item.id == lesson.id), 0)
    state = _lesson_state_for_user(current_user, module, lesson, idx)
    if state == 'locked' and current_user.role == UserRole.STUDENT:
...
def leaderboard(current_user: User):
    ...
    for idx, student in enumerate(students, start=1):
        payload.append({'position': idx, 'username': student.username, 'avatar': student.avatar, 'xp': student.xp, 'level': student.level, 'age_group': student.age_group})
...
@auth_required()
def forum_posts(current_user: User):
...
def create_post(current_user: User):
    ...
    post = ForumPost(
        ...
        title=data.get('title', 'Новый вопрос'),
        body=data.get('body', 'Опишите свой вопрос'),
    )
    db.session.add(post)
    db.session.commit()
...
@student_bp.patch('/forum/posts/<int:post_id>/thanks')
@auth_required()
def thank_post(current_user: User, post_id: int):
...
def update_profile(current_user: User):
    ...
    if 'companion' in data and data['companion']:
        current_user.companion = data['companion']
