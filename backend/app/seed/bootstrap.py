    ...
    for group_index, module_data in enumerate(junior_modules + middle_modules + senior_modules, start=1):
        ...
        for lesson_index, lesson_payload in enumerate(module_data['lessons'], start=1):
            ...
            db.session.add(
                Task(
                    lesson_id=lesson.id,
                    task_type='code' if lesson_payload['task'].get('starter_code') else 'text',
                    title=lesson_payload['task']['title'],
                    prompt=lesson_payload['task']['prompt'],
                    starter_code=lesson_payload['task'].get('starter_code', ''),
                    validation=lesson_payload['task']['validation'],
                    hints=['Проверь ключевые слова из темы.', 'Посмотри на пример выше и сократи решение.', 'Сначала напиши основной шаг, потом улучши формулировку.'],
                    xp_reward=30,
                )
            )
            db.session.add(
