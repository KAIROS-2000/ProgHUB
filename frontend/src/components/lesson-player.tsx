...
import { LessonDetail, QuizItem, QuizQuestion } from '@/types'
...
export function LessonPlayer({ lessonId }: { lessonId: number }) {
  ...
  useEffect(() => {
    api<{ lesson: LessonDetail }>(`/lessons/${lessonId}`, undefined, true)
      .then((data) => {
        setLesson(data.lesson)
        setAnswer(data.lesson.tasks[0]?.starter_code || '')
        setQuizAnswers({})
        setShownHints(0)
        setTheoryMarked(false)
        setInteractiveMarked(false)
        setTaskPassed(false)
        setQuizPassed(false)
        setResult('')
  ...
  async function submitTask() {
    ...
    setLoadingTask(true)
    try {
      const data = await api<{ feedback: string; score: number; passed: boolean; xp_awarded: number }>(`/tasks/${task.id}/submit`, {
        method: 'POST',
  ...
  async function submitQuiz() {
    ...
    setLoadingQuiz(true)
    try {
      const data = await api<{ passed: boolean; score: number; correct_answers: number; total_questions: number; xp_awarded: number }>(`/quizzes/${quiz.id}/submit`, {
        method: 'POST',
  ...
  const isJunior = lesson.module.age_group === 'junior'
