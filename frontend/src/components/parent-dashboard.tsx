    ...
    <div className="space-y-6">
      <section className="codequest-card overflow-hidden p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black text-slate-900">Прогресс: {data.child.full_name}</h1>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          ...
          ['Посты', String(data.summary.forum_posts)],
        ].map(([label, value]) => (
        ...
        <article className="codequest-card p-6">
          ...
          <div className="mt-5 grid gap-3">
            {data.weekly_activity.map((day) => {
              ...
              return (
                <div key={day.date} className="grid grid-cols-[70px_1fr_120px] items-center gap-3">
                  <p className="text-sm font-semibold text-slate-600">{day.label}</p>
                  <div className="flex items-end gap-3 rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <p className="text-right text-sm font-semibold text-slate-700">{day.average_score}%</p>
                </div>
              )
            })}
          </div>
