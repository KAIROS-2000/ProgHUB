...
export function ProfileView() {
  ...
  async function load() {
    ...
    setForm({
      ...
      companion: profileResponse.user.companion,
  ...
  async function handleSave(event: FormEvent) {
    ...
    setMessage('')
    try {
      const result = await api<{ user: UserItem }>('/users/me', {
        method: 'PATCH',
      ...
      <section className="codequest-card p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-black text-slate-900">{profile.full_name}</h1>
            <p className="mt-2 text-lg text-slate-600">@{profile.username} · {profile.email}</p>
          </div>
          <div className="grid min-w-[280px] gap-3 rounded-[24px] bg-slate-900 p-5 text-white">
        ...
        <form onSubmit={handleSave} className="codequest-card p-6">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <label className="space-y-2">
              ...
              <input className="w-full rounded-2xl border border-slate-200 px-4 py-3" value={form.companion} onChange={(e) => setForm({ ...form, companion: e.target.value })} />
            </label>
            <label className="space-y-2">
              <span className="text-sm font-semibold text-slate-600">Аватар</span>
              <input className="w-full rounded-2xl border border-slate-200 px-4 py-3" value={form.avatar} onChange={(e) => setForm({ ...form, avatar: e.target.value })} />
            </label>
            <label className="space-y-2">
              <span className="text-sm font-semibold text-slate-600">Тема</span>
              <select className="w-full rounded-2xl border border-slate-200 px-4 py-3" value={form.theme} onChange={(e) => setForm({ ...form, theme: e.target.value as 'light' | 'dark' })}>
                <option value="light">Светлая</option>
                <option value="dark">Тёмная</option>
              </select>
            </label>
          </div>
          <button disabled={saving} className="mt-5 rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">
